...
class AttachSpringGravity: UIViewController {
    ...
    func configureDynamicKit() {
        ...
        gravity.gravityDirection = CGVectorMake(0.0, 0.5) //Thử đổi giá trị x, y của vector xem kết quả thế nào
    ...
    func configureAttachment() {
        ...
        square.addGestureRecognizer(UIPanGestureRecognizer(target: self, action: "onMoveDragPoint:"))
        
    }
    func onMoveDragPoint(sender: UIPanGestureRecognizer) {
        attachmentBehavior!.anchorPoint = sender.locationInView(self.view)
        square.center = attachmentBehavior!.anchorPoint
    }
